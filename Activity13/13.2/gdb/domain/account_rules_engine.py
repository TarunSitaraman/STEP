...
class AccountRulesEngine:
    ...
    def get_minimum_balance(cls, account_type: str) -> float:
        normalised_type = account_type.strip().upper() if account_type else ""
        return cls._RULES.get(normalised_type, {}).get("min_balance", 0.0)

    @classmethod
    def get_interest_rate(cls, account_type: str) -> float:
        normalised_type = account_type.strip().upper() if account_type else ""
        return cls._RULES.get(normalised_type, {}).get("interest_rate", 0.0)

    @classmethod
    def get_overdraft_limit(cls, account_type: str) -> float:
        normalised_type = account_type.strip().upper() if account_type else ""
        return cls._RULES.get(normalised_type, {}).get("overdraft_limit", 0.0)
